...
class EncodingTests extends AnyFunSuite {
  ...
  test("decodeExpr[Option :: Int :: TNil]([⋅⋅[_], F[_], A] => (_: Unit) => (fa: F[A]) => fa)") {
    val id: Option[Int] => Option[Int] =
      decodeExpr[Option :: Int :: TNil]([⋅⋅[_], F[_], A] => (_: Unit) => (fa: F[A]) => fa)
    assert(id(Some(1)) == Some(1))
  }

  test("decodeExpr[List :: List :: Int :: TNil]([⋅⋅[_], F[_], G[_], A] => (_: Unit) => (fa: F[A], ga: G[A]) => fa)") {
    val fst: (List[Int], List[Int]) => List[Int] =
      decodeExpr[List :: List :: Int :: TNil]([⋅⋅[_], F[_], G[_], A] => (_: Unit) => (fa: F[A], ga: G[A]) => fa)
    assert(fst(List(1, 2, 3), List(4)) == List(1, 2, 3))
  }
